...
class MarketplaceService {
  ...
  async createOffer(listingId, offerData) {
    const listing = await this.tradeListings.findOne({ 
      _id: new ObjectId(listingId),
      status: 'active'
    });

    if (!listing) {
      throw new Error('Listing no encontrado o no está activo');
    }

    if (listing.user_id === offerData.buyer_id) {
      throw new Error('No puedes hacer ofertas en tus propios listings');
    }
    ...
    const offer = {
      ...
      buyer_username: offerData.buyer_username,
      offer_items: offerData.offer_items || [], // Array de items ofrecidos
  ...
  async getOffersForListing(listingId) {
    return await this.tradeOffers
      .find({ listing_id: new ObjectId(listingId) })
      .sort({ created_at: -1 })
      .toArray();
  }

  async getOffersForUser(userId, type = 'received') {
    const field = type === 'received' ? 'seller_id' : 'buyer_id';
    return await this.tradeOffers
      .find({ [field]: userId })
      .sort({ created_at: -1 })
      .toArray();
    ...
    await this.tradeListings.updateOne(
      ...
      { 
        $set: { 
          status: 'completed',
          completed_at: new Date(),
          traded_items: offer.offer_items // Guardar items intercambiados
        } 
      }
    );
  ...
  async counterOffer(offerId, sellerId, counterData) {
    const offer = await this.tradeOffers.findOne({ _id: new ObjectId(offerId) });
    ...
    if (offer.seller_id !== sellerId) {
      throw new Error('No tienes permiso para contraoferta');
    }

    const counter = {
      counter_items: counterData.counter_items || [],
    ...
    await this.tradeOffers.updateOne(
      ...
      { 
        $set: { 
          status: 'countered',
          last_counter_items: counterData.counter_items
        },
        $push: { counter_offers: counter }
      }
    );
